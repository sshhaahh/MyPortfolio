    ...
    <div className='flex flex-col gap-6' >
        ...
        <div className='grid grid-cols-1 md:grid-cols-2  gap-8 mx-2 ' >
            {
                Object.values(projects).map((project,index)=>(
                    <div onClick={()=>popupHandler(project)} className='rounded-2xl thin-outline hover:scale-105' key={index}  >
                        <img className='cursor-pointer rounded-t-2xl ' src={`https://api.microlink.io/?url=${encodeURIComponent(project.link)}&screenshot=true&embed=screenshot.url`}/>
                        <div className='px-3 py-2 gap-x-2'>
                            <p className='text-xl md:text-2xl lg:text-3xl  font-bold hover:text-[#0000EE] hover:underline cursor-pointer'>{project.name}</p>
        ...
        {Object.keys(popup).length === 0?"":(
            ...
            <div className='rounded-2xl thin-outline bg-black relative'   >
                ...
                <img onClick={()=>linkHandler(popup.link)} className='cursor-pointer rounded-t-2xl ' src={`https://api.microlink.io/?url=${encodeURIComponent(popup.link)}&screenshot=true&embed=screenshot.url`}/>
                <div className='px-3 py-2 gap-x-2'>
