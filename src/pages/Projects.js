  ...
  return (
    <div className='flex flex-col gap-6' >
                ...
                Object.values(projects).map((project,index)=>(
                    <div onClick={()=>popupHandler(project)} className='rounded-2xl thin-outline hover:scale-105' key={index}  >
                        ...
                        <div className='px-3 py-2 gap-x-2'>
                            ...
                            <p className='opacity-80 text-sm md:text-base lg:text-lg p-2'>{`${project.description.slice(0,60)}....`} </p>
                        </div>
                    </div>
                ))
            }
        ...
        {Object.keys(popup).length === 0?"":(
            ...
            <div className='rounded-2xl thin-outline bg-[#262626] p-2 relative'   >
                <button onClick={()=>setPopup({})} className='absolute right-3 text-3xl bg-[#262626] rounded-2xl top-3'><IoMdCloseCircle /></button>
